...
class DatabaseArchiver extends PackagerBase {
	...
	public static function execute() {
		...
		if ( isset( $params['database_archive_path'] ) ) {
			...
			$database_archive_path           = nfd_sm_get_hashed_file_path( 'database', 'backup', 'zip' );
		...
		Status::set_status( __( 'Archiving database ...', 'nfd-site-migrator' ), 15, 'database' );
		...
		if ( $completed ) {
			Status::set_status( __( 'Done archiving the database', 'nfd-site-migrator' ), 25, 'database' );
			...
			// Set progress
			Status::set_status( __( 'Archiving database...', 'nfd-site-migrator' ), 18, 'database' );
